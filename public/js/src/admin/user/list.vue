    ...
    <div>
        <portal to="header-actions">
            ...
            <div class="header-secondary_actions">
                <button type="button" class="header-actions-link button-link" @click="refresh">Refresh</button>
            </div>
        </portal>
...
<!--                </ul>-->
                ...
                <div class="record_list-wrap record_list-users mt-8 mb-12">

                    <div class="record_list-col record_list-col-heading">Username</div>
                    <div class="record_list-col record_list-col-heading">Name</div>
                    <div class="record_list-col record_list-col-heading">Customer</div>
                    ...
                    <template v-for="user in users"
                        ...
                        <div class="record_list-col record_list-col-break"  :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">
                            {{ user.email }}
                            <span v-if="user.userId === $store.state.user.userId" class="pl-3 italic">
                                You
                            </span>
                        ...
                        <div class="record_list-col"
                             :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">{{ user.name }}</div>
                        <div class="record_list-col"
                             :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">
                            <template v-if="user.customer">
                                {{ user.customer.customerNumber }} –
                                {{ user.customer.name }}
                            </template>
                        ...
                        <div class="record_list-col"
                             :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">{{ user|accountStatus }}</div>
                        <div class="record_list-col record_list-col-break user_list-last_login"
                             :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">
                            <template v-if="user.loginCount > 0">
                                <local-time :datetime="user.lastLogin" /> ({{ user.loginCount }})
                            </template>
                        ...
                        <div class="record_list-col"
                             :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">
                            {{ availableRoles[user.roles[0]] }}</div>

                        <div class="record_list-col record_list-col-actions" :class="{ 'record_list-col-inactive' : (!user.active || !user.verified) }">
                            <router-link :to="{ name: 'admin-user-edit', params: { userId: user.userId } }"
                                         class="record_list-action">
                                Edit
                            </router-link>
